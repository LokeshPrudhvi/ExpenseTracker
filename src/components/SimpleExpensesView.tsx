...
export function SimpleExpensesView({ 
  ...
  const categories = Array.from(new Set(monthExpenses.map(exp => exp.category)));
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg">
        <p className="text-sm text-green-900 dark:text-green-100">
          💰 Showing {sortedExpenses.length} expenses • Total: {currency} {totalAmount.toLocaleString()}
        </p>
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  <span className="text-3xl mt-1">{getCategoryIcon(expense.category)}</span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setDeleteId(expense._id || expense.id)} // ✅ Fixed: Use _id or id
